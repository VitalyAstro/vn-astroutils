...
if (len(sys.argv) > 1):
    ...
    Xerr0=data0[1,:]
    Yerr0=data0[3,:]
    pyplot.errorbar(X0, Y0, xerr=Xerr0, yerr=Yerr0,fmt='o',color='b',label=sys.argv[1])
if (len(sys.argv) > 2):
    FileName = sys.argv[2]
    data1 = loadtxt(FileName, usecols=[0,1,2,3], unpack=True)
    X1=data1[0,:]
    Y1=data1[2,:]
    Xerr1=data1[1,:]
    Yerr1=data1[3,:]
    pyplot.errorbar(X1, Y1, xerr=Xerr1, yerr=Yerr1,fmt='o',color='r',label=sys.argv[2])
#    legend.add(sys.argv[2])
if (len(sys.argv) > 3):
    FileName = sys.argv[3]
    data2 = loadtxt(FileName, usecols=[0,1,2,3], unpack=True)
    X2=data2[0,:]
    Y2=data2[2,:]
    Xerr2=data2[1,:]
    Yerr2=data2[3,:]
    pyplot.errorbar(X2, Y2, xerr=Xerr2, yerr=Yerr2,fmt='o',color='g',label=sys.argv[3])
#    legend.add((sys.argv[3]))
if (len(sys.argv) > 4):
    FileName = sys.argv[4]
    data3 = loadtxt(FileName, usecols=[0,1,2,3], unpack=True)
    X3=data3[0,:]
    Y3=data3[2,:]
    Xerr3=data3[1,:]
    Yerr3=data3[3,:]
    pyplot.errorbar(X3, Y3, xerr=Xerr3, yerr=Yerr3,fmt='o',color='k',label=sys.argv[4])
